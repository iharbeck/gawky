...
public class Generator
{
	...
	public String generateInsertSQL(Table bean)
	{
		...
		for(int i = 0; i < descs.length; i++)
		{
			...
			if(desc == descid) // && bean.getIdgenerator() != null) // ID Element überspringen
				continue;
	...
	public String generateCreateSQL(Table bean)
	{
		...
		for(int i = 0; i < descs.length; i++)
		{
			...
			if(desc == descid) // ID Element überspringen
				continue;
			...
			if(i < descs.length-1) { // beim letzten ohne Komma
				sql    += ",";
			} else if(descid != null) { // ID definiert
			    sql    += "," + descid.dbname;
			} 
		}
...
//			}
	...
	public void fillPreparedStatement(PreparedStatement stmt, Table bean, boolean insert)
	{
		...
		for(int i = 0; i < descs.length; i++)
		{
			...
			try 
			{
				if(desc == descid) // ID überspringen
					continue;
		...
		try {
			...
			if(!insert || (insert && bean.getParameter()))
				stmt.setString(setter+1, descid.getValue(bean)); 
