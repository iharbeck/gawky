...
public class Runner {
	...
	try {
			...
			try {
				...
				// FIXED BUG 2: The matching of email to certificate shuould not
				// be case sensitive
...
*/
	...
	private MimeMultipart sign(MimeBodyPart msg) {
		...
		try {
			...
			// add a signer to the generator - this confirms the use of SHA1.The
	 ...
	 * private PrivateKey getKey(String keyFile,String alias) {
	 * 
	 * try { //get keystore KeyStore ks = KeyStore.getInstance("PKCS12"); char []
	 * password = alias.toCharArray(); ks.load(new FileInputStream(keyFile),
	 * password); System.out.println("OK");
	 * 
	 * 
	 * 
	 * PrivateKey privatekey = null;
	 * 
	 * Enumeration en = ks.aliases(); String ALIAS = ""; Vector vectaliases =
	 * new Vector();
	 * 
	 * while (en.hasMoreElements()) vectaliases.add(en.nextElement()); String[]
	 * aliases = (String []) (vectaliases.toArray(new String[0])); for (int i =
	 * 0; i < aliases.length; i++) if (ks.isKeyEntry(aliases)) { ALIAS =
	 * aliases; break; }
	 * 
	 * privatekey = (PrivateKey)ks.getKey(ALIAS, password);
	 * System.out.println("alias :["ALIAS"] length:"+ALIAS.length());
	 * System.out.println("Private Key :"+privatekey.toString()); return
	 * privatekey; }catch(Exception exp){ exp.printStackTrace(); } return null; }
	 */
	/**
